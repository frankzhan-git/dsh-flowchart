...
export async function createFlowchartService({ storagesRoot, fsImpl }) {
  ...
  // 跨实例架构（v0.2.4-修复）：不持有 meta 缓存——listMeta/getMeta 每次直接读磁盘，
  // 同一 DSH_HOME 的多个 DSH 实例（:3080/:3090）互相可见（写入方落盘即对全部读方可见）
  let closed = false
  ...
  // 实时扫描：读取目录全部画布的 meta（文件为唯一权威，绝不过期）
  ...
  const service = {
    ...
    getMeta: async (id) => {
      ...
      return ok({ meta: await readMetaOf(id) })
    },
