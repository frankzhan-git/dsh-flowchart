...
## 0.2.8 (2026) — 修复：组内箭头不可选中 + 嵌套 subgraph 唯一归属

- **fix(命中优先，组容器语义)**：组合矩形内部不再遮挡其内成员箭头——
  `geometry.hitPriority` 重构为分层命中：节点表面（角/边带）→ 非组节点内部 → **箭头（路径 8/zoom 或标签矩形内）→ 组内部（兜底）**；
  此前组矩形内部（=大容器 area）先于箭头命中，组内箭头的"点击选中/锚点调整/右键删除/双击编辑"全部失效（0.2.7 组功能引入的回归）。
- **fix(事件路由统一)**：双击/右键从各组件 DOM 事件改为 **svg 层按坐标决策路由**（与选择同一 `hitPriority`）——
  组合矩形（透明但 painted fill）不再抢占其内成员箭头的事件面；右键命中间距/组合/页面/空白语义不变。
- **fix(绘制模式语义保持)**：绘制模式下点中组内箭头 → 仍按"组内部"处理（组移动），不误触空白建节点；
  多选状态（组+成员全选）下点击组内箭头 → 选中箭头（多选框不再吞箭头）。
- **fix(箭头标签命中带)**：`edgeLabelRect` 成为渲染与命中**单一来源**——标签矩形（±10px 高、含两端延伸）现在也算箭头命中，
  此前点中标签视觉区（弧线边尤为明显）会落空成框选。
- **feat(hover)**：选择模式贴近箭头 → `pointer` 光标（此前无任何可选中提示）。
- **fix(嵌套唯一归属)**：`coveredByRect` 只收「顶层被覆盖对象」——覆盖绘制命中组时其成员随组归入新组，**不再重复收编**；
  `settleDrag` 建组后 `unbindFromOthers` 解除旧组对成员的引用（每成员至多一个父组）；
  `buildGroupNode` 清除 `dragTmp`（0.2.7 曾把运行时标志持久化进存储）。
- **fix(codegen 唯一归属防御/C1)**：`buildPageCode` 以 `ownershipMap` 解析唯一归属（歧义取最深父组；环/自指/跨页/孤儿剔除）
  后递归输出 subgraph——修复「重复归属下子图成员错落、输出随数组序漂移」；发现归一时输出 info issue。
- **fix(载入清洗)**：`sanitizeDoc` 载入 `applyOwnership` + 清除 `dragTmp`（存量脏数据自动收敛到不变式，修复数计入 dropped）。
- **schema**：节点契约补充 `group`/`children`（0.2.7 遗漏）。
- **验证**：verify-grouping 新增 4 段断言（覆盖建组只收顶层/唯一归属/环与孤儿/嵌套 subgraph 输出）、
  verify-interactions 新增组内箭头命中/标签命中/绘制模式兜底/多选不吞箭头/hover 光标（9 组断言）；9 套件全绿。
- **发布**：dsh-flowchart@0.2.8

## 0.2.7 (2026) — 组合控件（P8：大矩形包裹其他控件）

- **fix(批量)**：框选多控件的批量选框 resize/宽高调整修复为**全体成员跟随**——
  - 外框边带/右下角批量语义从「仅选择模式」放开到**两种模式一致**（绘制模式下原先命中到边框上某个控件，表现为只有一个控件跟随）；
  - 批量边带调整支持**四边**（原先 t/l 边空补丁不生效），l/t 边 = x/y 随动、对缘固定；
  - `applyPatches` 补丁缺省字段保留原值（原先边带补丁只带 w/h，会把成员 x/y 覆盖为 undefined，控件漂移到原点）；
  - 多选外框边带/角 hover 显示方向光标（两种模式一致）。
- **fix(组合文本)**：组合矩形文本在画布中显示于**矩形顶部**（左右居中、与顶边保留 8px 间距，多行向下顺排）；普通控件保持竖横双居中。
- **feat(组合)**：绘制矩形覆盖其他控件 → 松开**自动成组**（大矩形透明化，成员包裹其内并整体框选）；无覆盖 → 普通节点（原行为）。
- **feat(拖入)**：将控件 A 拖入组合矩形 B → B **亮边框反馈**；松手 → A 归入 B（B 自动扩展至完全包裹，绝无部分重叠）。
- **feat(拖出)**：成员拖出组外 → 解除归属；拖动组合 → 内部成员整体跟随；组合 resize → 成员等比缩放（始终完全包裹）。
- **feat(编码)**：组合 → mermaid `subgraph`（成员行递归输出，组 id 可直接参与箭头）；删除组合只删外框（成员保留散出）。
- **验证**：verify-grouping 8 段断言（几何/成组/扩组/解除/推离/排序/subgraph）+ headless 实操（绘制成组/拖入亮边/拖出解除）全通过。
- **发布**：dsh-flowchart@0.2.7

## 0.2.6 (2026) — 跨实例同步及时性（列表自动刷新）

- **feat(同步)**：弹窗打开期间每 4s 静默轮询 `listMeta` + 页面聚焦/切回时立即刷新——多实例（:3080/:3090）场景无跨进程事件推送，轮询让对面的新增/删除/改名**自动出现在已打开的列表**（不再需要手动重开弹窗/刷新页面）。
- **chore**：verify 8 套件 + 网关级预验证全绿。
- **发布**：dsh-flowchart@0.2.6
